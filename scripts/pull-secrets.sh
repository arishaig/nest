...
{
cat << 'HEADER'
# Auto-generated by pull-secrets.sh
# Encrypt this file: ansible-vault encrypt inventory/group_vars/all/vault.yml
...
# Count how many are still CHANGEME
REMAINING=$(grep -c 'CHANGEME' "$VAULT_FILE" || true)
if [[ "$REMAINING" -gt 0 ]]; then
    warn "$REMAINING values still set to CHANGEME — review and fill in manually:"
    grep -n 'CHANGEME' "$VAULT_FILE" | sed 's/^/  /'
fi

echo ""
echo "Next steps:"
echo "  1. Review the file: less $VAULT_FILE"
echo "  2. Fill in any CHANGEME values manually"
echo "  3. Encrypt it: ansible-vault encrypt $VAULT_FILE"
echo ""
